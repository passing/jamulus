                        ...
                        const char* jackClientName )
{
    ...
    pJackClient =
        jack_client_open ( jackClientName, JackNullOption, &JackStatus );

    if ( pJackClient == nullptr )
    {
        throw CGenErr ( tr (
            "The Jack server is not running. This software "
            "requires a Jack server to run. Normally if the Jack server is "
            "not running this software will automatically start the Jack "
            "server. "
            "It seems that this auto start has not worked. Try to start the "
            "Jack "
            "server manually." ) );
    }
    ...
    input_port_right = jack_port_register ( pJackClient,
                                            "input right",
                                            JACK_DEFAULT_AUDIO_TYPE,
                                            JackPortIsInput,
                                            0 );

    output_port_left = jack_port_register ( pJackClient,
                                            "output left",
                                            JACK_DEFAULT_AUDIO_TYPE,
                                            JackPortIsOutput,
                                            0 );

    output_port_right = jack_port_register ( pJackClient,
                                             "output right",
                                             JACK_DEFAULT_AUDIO_TYPE,
                                             JackPortIsOutput,
                                             0 );

    if ( ( input_port_left == nullptr ) || ( input_port_right == nullptr ) ||
    ...
    {
        input_port_midi = jack_port_register ( pJackClient,
                                               "input midi",
    ...
    if ( !bNoAutoJackConnect )
    {
                                            ...
                                            JackPortIsOutput ) ) != nullptr )
        {
            ...
            if ( ports[1] )
            {
                jack_connect ( pJackClient,
                               ports[1],
                               jack_port_name ( input_port_right ) );
            }
                                            ...
                                            JackPortIsInput ) ) != nullptr )
        {
            ...
            {
                jack_connect ( pJackClient,
                               jack_port_name ( output_port_right ),
                               ports[1] );
            }
        ...
        jack_port_get_latency_range ( input_port_left,
                                      JackCaptureLatency,
                                      &latrange );
        ...
        jack_port_get_latency_range ( output_port_left,
                                      JackPlaybackLatency,
                                      &latrange );
        ...
        fInOutLatencyMs = static_cast<float> ( inLatency + outLatency ) * 1000 /
...
int CSound::Init ( const int /* iNewPrefMonoBufferSize */ )
{
    ...
    {
        throw CGenErr ( tr (
            "The Jack server was shut down. This software "
...
int CSound::process ( jack_nframes_t nframes, void* arg )
{
    ...
    if ( pSound->IsRunning() &&
         ( nframes ==
           static_cast<jack_nframes_t> ( pSound->iJACKBufferSizeMono ) ) )
    {
        // get input data pointer
        jack_default_audio_sample_t* in_left = (jack_default_audio_sample_t*)
            jack_port_get_buffer ( pSound->input_port_left, nframes );

        jack_default_audio_sample_t* in_right = (jack_default_audio_sample_t*)
            jack_port_get_buffer ( pSound->input_port_right, nframes );
        ...
        if ( ( in_left != nullptr ) && ( in_right != nullptr ) )
        {
            for ( i = 0; i < pSound->iJACKBufferSizeMono; i++ )
            {
                pSound->vecsTmpAudioSndCrdStereo[2 * i] =
                    Float2Short ( in_left[i] * _MAXSHORT );
                pSound->vecsTmpAudioSndCrdStereo[2 * i + 1] =
                    Float2Short ( in_right[i] * _MAXSHORT );
            }
        }
        ...
        jack_default_audio_sample_t* out_left = (jack_default_audio_sample_t*)
            jack_port_get_buffer ( pSound->output_port_left, nframes );

        jack_default_audio_sample_t* out_right = (jack_default_audio_sample_t*)
            jack_port_get_buffer ( pSound->output_port_right, nframes );
    ...
    {
        void* in_midi =
            jack_port_get_buffer ( pSound->input_port_midi, nframes );
        ...
        if ( in_midi != 0 )
        {
            ...
            for ( jack_nframes_t j = 0; j < event_count; j++ )
            {
                ...
                {
                    vMIDIPaketBytes[i] =
                        static_cast<uint8_t> ( in_event.buffer[i] );
                }
