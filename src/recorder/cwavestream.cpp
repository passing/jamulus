 ...
 */
recorder::CWaveStream& operator<< ( recorder::CWaveStream&   os,
                                    const recorder::HdrRiff& obj )
{
 ...
 */
recorder::CWaveStream& operator<< ( recorder::CWaveStream&       os,
                                    const recorder::FmtSubChunk& obj )
{
    (QDataStream&) os << obj.chunkId << obj.chunkSize << obj.audioFormat
                      << obj.numChannels << obj.sampleRate << obj.byteRate
                      << obj.blockAlign << obj.bitsPerSample;
    return os;
}
 ...
 */
recorder::CWaveStream& operator<< ( recorder::CWaveStream&           os,
                                    const recorder::DataSubChunkHdr& obj )
{
...
CWaveStream::CWaveStream ( QByteArray*         iod,
...
{
    static const uint64_t hdrRiffChunkSize =
        sizeof ( uint32_t ) + sizeof ( uint32_t ) + sizeof ( uint32_t );
    static const uint64_t fmtSubChunkSize =
    ...
    static const uint64_t dataSubChunkHdrChunkSizeOffset =
        hdrRiffChunkSize + fmtSubChunkSize + sizeof ( uint32_t );

    const int64_t  currentPos     = this->device()->pos();
    const uint64_t fileLengthRiff = static_cast<uint64_t> (
        currentPos - initialPos -
        ( hdrRiffChunkSizeOffset + sizeof ( uint32_t ) ) );
    const uint64_t fileLengthData = static_cast<uint64_t> (
        currentPos - initialPos -
        ( dataSubChunkHdrChunkSizeOffset + sizeof ( uint32_t ) ) );
