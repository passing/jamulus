...
{
    return QString::number ( static_cast<double> ( frames * frameSize ) / 48000,
                             'f',
                             14 );
}
...
class FmtSubChunk
{
  public:
    ...
    static const uint32_t chunkSize =
        16; // bytes in fmtSubChunk after chunkSize
...
class DataSubChunkHdr
{
  public:
    ...
    static const uint32_t chunkSize =
        0x7ffff000; // magic for unspecified length
};
...
recorder::CWaveStream& operator<< ( recorder::CWaveStream& out,
                                    recorder::HdrRiff&     hdrRiff );
recorder::CWaveStream& operator<< ( recorder::CWaveStream& out,
                                    recorder::FmtSubChunk& fmtSubChunk );
recorder::CWaveStream&
operator<< ( recorder::CWaveStream&     out,
             recorder::DataSubChunkHdr& dataSubChunkHdr );
