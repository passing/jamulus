...
class CSoundBase : public QThread
{
  ...
  public:
    CSoundBase ( const QString& strNewSystemDriverTechniqueName,
                 void ( *fpNewProcessCallback ) ( CVector<int16_t>& psData,
                                                  void*             pParg ),
                 void*          pParg,
    ...
    virtual float GetInOutLatencyMs()
    {
        return 0.0f;
    } // "0.0" means no latency is available
  ...
  protected:
    ...
    static void GetSelCHAndAddCH ( const int iSelCH,
                                   const int iNumInChan,
                                   int&      iSelCHOut,
                                   int&      iSelAddCHOut )
    {
