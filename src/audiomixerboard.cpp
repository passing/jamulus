...
* CChanneFader                                                                 *
\******************************************************************************/
CChannelFader::CChannelFader ( QWidget* pNW ) : eDesign ( GD_STANDARD )
{
    ...
    pGroupPopupMenu->addAction ( tr ( "&No grouping" ),
                                 this,
                                 SLOT ( OnGroupMenuGrpNone() ) );
    pGroupPopupMenu->addAction ( tr ( "Assign to group" ) + " &1",
                                 this,
                                 SLOT ( OnGroupMenuGrp1() ) );
    pGroupPopupMenu->addAction ( tr ( "Assign to group" ) + " &2",
                                 this,
                                 SLOT ( OnGroupMenuGrp2() ) );
    pGroupPopupMenu->addAction ( tr ( "Assign to group" ) + " &3",
                                 this,
                                 SLOT ( OnGroupMenuGrp3() ) );
    pGroupPopupMenu->addAction ( tr ( "Assign to group" ) + " &4",
                                 this,
                                 SLOT ( OnGroupMenuGrp4() ) );
#if ( MAX_NUM_FADER_GROUPS != 4 )
#error                                                                         \
    "MAX_NUM_FADER_GROUPS must be set to 4, see implementation in CChannelFader()"
#endif
    ...
    pInfoLabel->setMinimumHeight (
        15 ); // prevents jitter when muting/unmuting (#811)
    ...
    pLabelGrid->addWidget (
        plblLabel,
        0,
        Qt::AlignVCenter ); // note: just initial add, may be changed later
    ...
    // reset current fader
    strGroupBaseText =
        "Grp"; // this will most probably overwritten by SetGUIDesign()
    iInstrPicMaxWidth =
        INVALID_INDEX; // this will most probably overwritten by SetGUIDesign()
    Reset();

    // add help text to controls
    plbrChannelLevel->setWhatsThis (
        "<b>" + tr ( "Channel Level" ) + ":</b> " +
        tr ( "Displays the pre-fader audio level of this channel.  All clients "
             "connected to the "
             "server will be assigned an audio level, the same value for every "
             "client." ) );
    plbrChannelLevel->setAccessibleName (
        tr ( "Input level of the current audio "
             "channel at the server" ) );

    pFader->setWhatsThis ( "<b>" + tr ( "Mixer Fader" ) + ":</b> " +
                           tr ( "Adjusts the audio level of this channel. All "
                                "clients connected to the server "
                                "will be assigned an audio fader, displayed at "
                                "each client, to adjust the local mix." ) );
    pFader->setAccessibleName (
        tr ( "Local mix level setting of the current audio "
             "channel at the server" ) );

    pInfoLabel->setWhatsThis (
        "<b>" + tr ( "Status Indicator" ) + ":</b> " +
        tr ( "Shows a status indication about the client which is assigned to "
             "this channel. "
             "Supported indicators are:" ) +
        "<ul><li>" +
        tr ( "Speaker with cancellation stroke: Indicates that another client "
             "has muted you." ) +
        "</li></ul>" );
    ...
    pPan->setAccessibleName ( tr (
        "Local panning position of the current audio channel at the server" ) );

    pcbMute->setWhatsThis (
        "<b>" + tr ( "Mute" ) + ":</b> " +
        tr ( "With the Mute checkbox, the audio channel can be muted." ) );
    pcbMute->setAccessibleName ( tr ( "Mute button" ) );

    pcbSolo->setWhatsThis ( "<b>" + tr ( "Solo" ) + ":</b> " +
                            tr ( "With the Solo checkbox, the "
                                 "audio channel can be set to solo which means "
                                 "that all other channels "
                                 "except the soloed channel are muted. It is "
    ...
    pcbGroup->setWhatsThis ( "<b>" + tr ( "Group" ) + ":</b> " +
                             tr ( "With the Grp checkbox, a "
                                  "group of audio channels can be defined. All "
                                  "channel faders in a group are moved "
                                  "in proportional synchronization if any one "
                                  "of the group faders are moved." ) );
    pcbGroup->setAccessibleName ( tr ( "Group button" ) );

    QString strFaderText =
        "<b>" + tr ( "Fader Tag" ) + ":</b> " +
        tr ( "The fader tag "
             "identifies the connected client. The tag name, a picture of your "
             "instrument and the flag of your country can be set in the main "
             "window." );

    plblInstrument->setWhatsThis ( strFaderText );
    plblInstrument->setAccessibleName (
        tr ( "Mixer channel instrument picture" ) );
    ...
    // Connections -------------------------------------------------------------
    QObject::connect ( pFader,
                       &QSlider::valueChanged,
                       this,
                       &CChannelFader::OnLevelValueChanged );

    QObject::connect ( pPan,
                       &QDial::valueChanged,
                       this,
                       &CChannelFader::OnPanValueChanged );

    QObject::connect ( pcbMute,
    ...
    QObject::connect ( pcbSolo,
                       &QCheckBox::stateChanged,
                       this,
                       &CChannelFader::soloStateChanged );

    QObject::connect ( pcbGroup,
                       &QCheckBox::stateChanged,
                       this,
                       &CChannelFader::OnGroupStateChanged );
}
...
void CChannelFader::SetGUIDesign ( const EGUIDesign eNewDesign )
{
    ...
    case GD_ORIGINAL:
        pFader->setStyleSheet (
            "QSlider { width:         45px;"
            "          border-image:  url(:/png/fader/res/faderbackground.png) "
            "repeat;"
        ...
        pLabelGrid->addWidget ( plblLabel,
                                0,
                                Qt::AlignVCenter ); // label next to icons
    ...
    case GD_SLIMFADER:
        pLabelPictGrid->addWidget ( plblLabel,
                                    0,
                                    Qt::AlignHCenter ); // label below icons
        pLabelInstBox->setMinimumHeight (
            130 ); // maximum height of the instrument+flag+label
...
void CChannelFader::SetupFaderTag ( const ESkillLevel eSkillLevel )
{
    ...
    QString strStile = "QGroupBox { border:        2px solid " +
                       strBorderColor +
                       ";"
                       "            border-radius: 4px;"
                       "            padding:       3px;";
                                    ...
                                    const bool   bIsGroupUpdate )
{
    ...
    if ( dLevel >= 0 )
    {
        ...
        SendFaderLevelToServer (
            min ( static_cast<double> ( AUD_MIX_FADER_MAX ), dLevel ),
            bIsGroupUpdate );
...
void CChannelFader::SetMute ( const bool bState )
{
    ...
    else
    {
        ...
        if ( ( !bOtherChannelIsSolo || IsSolo() ) && bIsMutedAtServer )
        {
            ...
            emit gainValueChanged (
                MathUtils::CalcFaderGain ( GetFaderLevel() ),
                bIsMyOwnFader,
                false,
                false,
                -1 ); // set level ratio to in invalid value
...
void CChannelFader::SetChannelInfos ( const CChannelInfo& cChanInfo )
{
    ...
    if ( eDesign == GD_SLIMFADER )
    {
        ...
        for ( int iInsPos = 4; iInsPos <= strModText.size() - 1;
              iInsPos += 4 + 1 )
        {
    ...
    if ( cChanInfo.eCountry != QLocale::AnyCountry )
    {
        ...
        QPixmap CountryFlagPixmap (
            CLocale::GetCountryFlagIconsResourceReference (
                cChanInfo.eCountry ) );
    ...
    {
        strToolTip +=
            "<h4>" + tr ( "Alias/Name" ) + "</h4>" + cChanInfo.strName;
    ...
    eChSortType ( ST_NO_SORT )
{
    ...
    setWhatsThis ( "<b>" + tr ( "Personal Mix at the Server" ) + ":</b> " +
                   tr ( "When connected to a server, the controls here allow "
                        "you to set your "
                        "local mix without affecting what others hear from "
                        "you. The title shows "
                        "the server name and, when known, whether it is "
                        "actively recording." ) );
    ...
    pGroupBoxLayout->setContentsMargins (
        0,
        0,
        0,
    ...
    pScrollArea->setMinimumWidth (
        200 ); // at least two faders shall be visible
    pScrollArea->setWidget ( pMixerWidget );
    pScrollArea->setWidgetResizable (
        true ); // make sure it fills the entire scroll area
...
inline void CAudioMixerBoard::connectFaderSignalsToMixerBoardSlots()
{
    ...
    void ( CAudioMixerBoard::*pGainValueChanged ) ( float,
                                                    bool,
                                                    bool,
                                                    bool,
                                                    double ) =
    ...
    QObject::connect ( vecpChanFader[iCurChanID],
                       &CChannelFader::soloStateChanged,
                       this,
                       &CAudioMixerBoard::UpdateSoloStates );

    QObject::connect ( vecpChanFader[iCurChanID],
                       &CChannelFader::gainValueChanged,
                       this,
                       pGainValueChanged );

    QObject::connect ( vecpChanFader[iCurChanID],
                       &CChannelFader::panValueChanged,
                       this,
                       pPanValueChanged );
...
void CAudioMixerBoard::ChangeFaderOrder ( const EChSortType eChSortType )
{
    ...
    for ( int i = 0; i < MAX_NUM_CHANNELS; i++ )
    {
        ...
        {
            PairList << QPair<QString, int> (
                vecpChanFader[i]->GetReceivedCity().toLower(),
                i );
        }
        ...
        else if ( eChSortType == ST_BY_GROUPID )
        {
            ...
            {
                PairList << QPair<QString, int> (
                    QString::number ( vecpChanFader[i]->GetGroupID() ),
                    i );
            }
        }
        else // ST_NO_SORT
...
void CAudioMixerBoard::SetRecorderState (
    const ERecorderState newRecorderState )
{
...
void CAudioMixerBoard::ApplyNewConClientList (
                ...
                if ( vecChanInfo[j].iChanID == i )
                {
                    ...
                    if ( !vecpChanFader[i]->IsVisible() )
                    {
                             ...
                             ( pSettings->iNewClientFaderLevel != 100 ) )
                        {
                            ...
                            vecpChanFader[i]->SetFaderLevel (
                                pSettings->iNewClientFaderLevel / 100.0 *
                                AUD_MIX_FADER_MAX );
                        }
                    }
                             ...
                             vecChanInfo[j].strName ) )
                    {
                                                      ...
                                                      iGroupID ) )
                        {
                            vecpChanFader[i]->SetFaderLevel (
                                iStoredFaderLevel,
                                true ); // suppress group update
                            vecpChanFader[i]->SetPanValue ( iStoredPanValue );
                            vecpChanFader[i]->SetFaderIsSolo (
                                bStoredFaderIsSolo );
...
void CAudioMixerBoard::SetAllFaderLevelsToNewClientLevel()
{
    ...
    for ( int i = 0; i < MAX_NUM_CHANNELS; i++ )
    {
        ...
        if ( vecpChanFader[i]->IsVisible() && ( i != iMyChannelID ) )
        {
            ...
            vecpChanFader[i]->SetFaderLevel ( pSettings->iNewClientFaderLevel /
                                                  100.0 * AUD_MIX_FADER_MAX,
                                              true );
        }
    }
}
                                         ...
                                         const double dLevelRatio )
{
         ...
         !bIsGroupUpdate )
    {
        for ( int i = 0; i < MAX_NUM_CHANNELS; i++ )
        {
            ...
            if ( vecpChanFader[i]->IsVisible() &&
                 ( vecpChanFader[i]->GetGroupID() ==
                   vecpChanFader[iChannelIdx]->GetGroupID() ) &&
                 ( i != iChannelIdx ) && ( dLevelRatio >= 0 ) )
            {
                ...
                vecpChanFader[i]->SetFaderLevel (
                    vecpChanFader[i]->GetPreviousFaderLevel() * dLevelRatio,
                    true );
            }
        }
    }
...
void CAudioMixerBoard::StoreFaderSettings ( CChannelFader* pChanFader )
{
    ...
    if ( pChanFader->IsVisible() && !pChanFader->GetReceivedName().isEmpty() )
    {
        ...
        CVector<int> vbOldStoredFaderGroupID (
            pSettings->vecStoredFaderGroupID );

        // put new value on the top of the list
        const int iOldIdx =
            pSettings->vecStoredFaderTags.StringFiFoWithCompare (
                pChanFader->GetReceivedName() );
        ...
        int iTempListCnt =
            1; // current fader is on top, other faders index start at 1
            ...
            if ( iTempListCnt < MAX_NUM_STORED_FADER_SETTINGS )
            {
                ...
                if ( iIdx != iOldIdx )
                {
                    pSettings->vecStoredFaderLevels[iTempListCnt] =
                        viOldStoredFaderLevels[iIdx];
                    pSettings->vecStoredPanValues[iTempListCnt] =
                        viOldStoredPanValues[iIdx];
                    pSettings->vecStoredFaderIsSolo[iTempListCnt] =
                        vbOldStoredFaderIsSolo[iIdx];
                    pSettings->vecStoredFaderIsMute[iTempListCnt] =
                        vbOldStoredFaderIsMute[iIdx];
                    pSettings->vecStoredFaderGroupID[iTempListCnt] =
                        vbOldStoredFaderGroupID[iIdx];
...
void CAudioMixerBoard::SetChannelLevels (
    const CVector<uint16_t>& vecChannelLevel )
{
