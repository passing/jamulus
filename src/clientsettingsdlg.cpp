    ...
    pSettings ( pNSetP )
{
    ...
    // jitter buffer
    QString strJitterBufferSize =
        "<b>" + tr ( "Jitter Buffer Size" ) + ":</b> " +
        tr ( "The jitter buffer compensates for network and sound card timing "
             "jitters. The "
    ...
    sldNetBuf->setAccessibleName (
        tr ( "Local jitter buffer slider control" ) );
    sldNetBuf->setToolTip ( strJitterBufferSizeTT );
    sldNetBufServer->setWhatsThis ( strJitterBufferSize );
    sldNetBufServer->setAccessibleName (
        tr ( "Server jitter buffer slider control" ) );
    ...
    cbxSoundcard->setWhatsThis (
        "<b>" + tr ( "Sound Card Device" ) + ":</b> " +
        tr ( "The ASIO driver (sound card) can be selected using " ) +
        APP_NAME +
        tr ( " under the Windows operating system. Under MacOS/Linux, no sound "
             "card selection is possible. If the selected ASIO driver is not "
             "valid "
             "an error message is shown and the previous valid driver is "
             "selected." ) +
        "<br>" +
        tr ( "If the driver is selected during an active connection, the "
             "connection "
             "is stopped, the driver is changed and the connection is started "
             "again "
             "automatically." ) );

    cbxSoundcard->setAccessibleName (
        tr ( "Sound card device selector combo box" ) );

#ifdef _WIN32
    // set Windows specific tool tip
    cbxSoundcard->setToolTip ( tr ( "If the ASIO4ALL driver is used, "
                                    "please note that this driver usually "
                                    "introduces approx. 10-30 ms of "
                                    "additional audio delay. Using a sound "
                                    "card with a native ASIO driver "
                                    "is therefore recommended." ) +
                               "<br>" +
                               tr ( "If you are using the kX ASIO "
                                    "driver, make sure to connect the ASIO "
                                    "inputs in the kX DSP settings "
                                    "panel." ) +
                               TOOLTIP_COM_END_TEXT );
#endif

    // sound card input/output channel mapping
    QString strSndCrdChanMapp =
        "<b>" + tr ( "Sound Card Channel Mapping" ) + ":</b> " +
    ...
    cbxLInChan->setAccessibleName (
        tr ( "Left input channel selection combo box" ) );
    cbxRInChan->setWhatsThis ( strSndCrdChanMapp );
    cbxRInChan->setAccessibleName (
        tr ( "Right input channel selection combo box" ) );
    cbxLOutChan->setWhatsThis ( strSndCrdChanMapp );
    cbxLOutChan->setAccessibleName (
        tr ( "Left output channel selection combo box" ) );
    cbxROutChan->setWhatsThis ( strSndCrdChanMapp );
    cbxROutChan->setAccessibleName (
        tr ( "Right output channel selection combo box" ) );

    // enable OPUS64
    chbEnableOPUS64->setWhatsThis (
        "<b>" + tr ( "Enable Small Network Buffers" ) + ":</b> " +
        tr ( "If enabled, the support for very small network audio packets is "
             "activated. Very small "
             "network packets are only actually used if the sound card buffer "
             "delay is smaller than " ) +
        QString().setNum ( DOUBLE_SYSTEM_FRAME_SIZE_SAMPLES ) +
        tr ( " samples. The "
             "smaller the network buffers, the lower the audio latency. But at "
             "the same time "
             "the network load increases and the probability of audio dropouts "
             "also increases." ) );

    chbEnableOPUS64->setAccessibleName (
        tr ( "Enable small network buffers check box" ) );
    ...
    QString strSndCrdBufDelay =
        ...
        "</li>"
        "</ul>" +
        tr ( "Some sound card drivers do not allow the buffer delay to be "
             "changed "
        ...
        tr ( "If no buffer size is selected and all settings are disabled, an "
             ...
             "performance." ) +
        "<br>" +
        tr ( "The actual buffer delay has influence on the connection status, "
             "the "
             "current upload rate and the overall delay. The lower the buffer "
             "size, "
             "the higher the probability of a red light in the status "
             "indicator (drop "
    ...
    QString strSndCrdBufDelayTT =
        tr ( "If the buffer delay settings are "
    ...
    // fancy skin
    cbxSkin->setWhatsThis (
        "<b>" + tr ( "Skin" ) + ":</b> " +
        tr ( "Select the skin to be used for the main window." ) );

    cbxSkin->setAccessibleName ( tr ( "Skin combo box" ) );

    // audio channels
    QString strAudioChannels =
        "<b>" + tr ( "Audio Channels" ) + ":</b> " +
        tr ( "Selects the number of audio channels to be used for "
             "communication between "
             "client and server. There are three modes available:" ) +
        "<ul>"
        "<li>"
        "<b>" +
        tr ( "Mono" ) + "</b> " + tr ( "and " ) + "<b>" + tr ( "Stereo" ) +
        ":</b> " +
        tr ( "These modes use "
        ...
        tr (
            " mode "
            "will increase your stream's data rate. Make sure your upload rate "
            "does not "
    ...
    // audio quality
    QString strAudioQuality =
        "<b>" + tr ( "Audio Quality" ) + ":</b> " +
    ...
    QString strNewClientLevel =
        "<b>" + tr ( "New Client Level" ) + ":</b> " +
    ...
    QString strCentrServAddr = "<b>" + tr ( "Custom Central Server Address" ) +
                               ":</b> " +
                               tr ( "Leave this blank unless you need to enter "
                                    "the address of a central "
    ...
    cbxCentralServerAddress->setAccessibleName (
        tr ( "Central server address combo box" ) );
    ...
    QString strConnStats =
        ...
        tr ( "The Ping Time is the time required for the audio "
             "stream to travel from the client to the server and back again. "
             "This "
             "delay is introduced by the network and should be about "
             "20-30 ms. If this delay is higher than about 50 ms, your "
             "distance to "
        ...
        tr ( "Audio Upstream Rate depends on the current audio packet size and "
             ...
             "higher than your available internet upload speed (check this "
             "with a "
    ...
    ledOverallDelay->setToolTip (
        tr ( "If this LED indicator turns red, "
    ...
    edtNewClientLevel->setValidator (
        new QIntValidator ( 0, 100, this ) ); // % range from 0-100

    // init slider controls ---
    // network buffer sliders
    sldNetBuf->setRange ( MIN_NET_BUF_SIZE_NUM_BL, MAX_NET_BUF_SIZE_NUM_BL );
    sldNetBufServer->setRange ( MIN_NET_BUF_SIZE_NUM_BL,
    ...
    cbxAudioChannels->addItem (
        tr ( "Mono-in/Stereo-out" ) );             // CC_MONO_IN_STEREO_OUT
    cbxAudioChannels->addItem ( tr ( "Stereo" ) ); // CC_STEREO
    cbxAudioChannels->setCurrentIndex (
    ...
    cbxAudioQuality->setCurrentIndex (
    ...
    edtNewClientLevel->setText (
        QString::number ( pSettings->iNewClientFaderLevel ) );

    // update enable small network buffers check box
    chbEnableOPUS64->setCheckState (
        pClient->GetEnableOPUS64() ? Qt::Checked : Qt::Unchecked );
    ...
    QObject::connect ( &TimerStatus,
                       &QTimer::timeout,
                       this,
                       &CClientSettingsDlg::OnTimerStatus );

    // slider controls
    QObject::connect ( sldNetBuf,
                       &QSlider::valueChanged,
                       this,
                       &CClientSettingsDlg::OnNetBufValueChanged );

    QObject::connect ( sldNetBufServer,
                       &QSlider::valueChanged,
                       this,
                       &CClientSettingsDlg::OnNetBufServerValueChanged );

    // check boxes
    QObject::connect ( chbAutoJitBuf,
                       &QCheckBox::stateChanged,
                       this,
                       &CClientSettingsDlg::OnAutoJitBufStateChanged );

    QObject::connect ( chbEnableOPUS64,
                       &QCheckBox::stateChanged,
                       this,
                       &CClientSettingsDlg::OnEnableOPUS64StateChanged );

    // line edits
    QObject::connect ( edtNewClientLevel,
                       &QLineEdit::editingFinished,
                       this,
                       &CClientSettingsDlg::OnNewClientLevelEditingFinished );
    ...
    QObject::connect (
        cbxCentralServerAddress->lineEdit(),
        &QLineEdit::editingFinished,
        this,
    ...
    QObject::connect ( cbxLanguage,
                       ...
                       this,
                       &CClientSettingsDlg::OnLanguageChanged );

    // buttons
    QObject::connect ( butDriverSetup,
                       &QPushButton::clicked,
                       this,
                       &CClientSettingsDlg::OnDriverSetupClicked );

    // misc
    QObject::connect (
        &SndCrdBufferDelayButtonGroup,
...
void CClientSettingsDlg::UpdateJitterBufferFrame()
{
    ...
    lblNetBufServer->setText ( tr ( "Size: " ) +
                               QString().setNum ( iCurNumNetBufServer ) );
...
QString
CClientSettingsDlg::GenSndCrdBufferDelayString ( const int     iFrameSize,
                                                 const QString strAddText )
{
    // use two times the buffer delay for the entire delay since
    // we have input and output
    return QString().setNum ( static_cast<double> ( iFrameSize ) * 2 * 1000 /
                                  SYSTEM_SAMPLE_RATE_HZ,
                              'f',
                              2 ) +
           " ms (" + QString().setNum ( iFrameSize ) + strAddText + ")";
}
...
void CClientSettingsDlg::UpdateSoundDeviceChannelSelectionFrame()
{
    ...
    else
    {
        ...
        cbxRInChan->clear();
        for ( iSndChanIdx = 0;
              iSndChanIdx < pClient->GetSndCrdNumInputChannels();
              iSndChanIdx++ )
        {
            cbxLInChan->addItem (
                pClient->GetSndCrdInputChannelName ( iSndChanIdx ) );
            cbxRInChan->addItem (
                pClient->GetSndCrdInputChannelName ( iSndChanIdx ) );
        }
        if ( pClient->GetSndCrdNumInputChannels() > 0 )
        {
            cbxLInChan->setCurrentIndex (
        ...
        cbxROutChan->clear();
        for ( iSndChanIdx = 0;
              iSndChanIdx < pClient->GetSndCrdNumOutputChannels();
              iSndChanIdx++ )
        {
            cbxLOutChan->addItem (
                pClient->GetSndCrdOutputChannelName ( iSndChanIdx ) );
            cbxROutChan->addItem (
                pClient->GetSndCrdOutputChannelName ( iSndChanIdx ) );
        }
...
void CClientSettingsDlg::OnCentralServerAddressEditingFinished()
{
    ...
    if ( cbxCentralServerAddress->currentText().isEmpty() &&
         cbxCentralServerAddress->currentData().isValid() )
    {
        pSettings->vstrCentralServerAddress
            [cbxCentralServerAddress->currentData().toInt()] = "";
    }
    else
    {
        // store new address at the top of the list, if the list was already
        // full, the last element is thrown out
        pSettings->vstrCentralServerAddress.StringFiFoWithCompare (
            NetworkUtil::FixAddress (
                cbxCentralServerAddress->currentText() ) );
    }
    ...
    const CMultiColorLED::ELightColor eOverallDelayLEDColor )
{
    ...
    if ( iPingTime > 500 )
    {
        const QString sErrorText = "<font color="
                                   "red"
                                   "><b>&#62;500 ms</b></font>";
...
void CClientSettingsDlg::UpdateCustomCentralServerComboBox()
{
    ...
    for ( int iLEIdx = 0; iLEIdx < MAX_NUM_SERVER_ADDR_ITEMS; iLEIdx++ )
    {
        if ( !pSettings->vstrCentralServerAddress[iLEIdx].isEmpty() )
        {
            ...
            cbxCentralServerAddress->addItem (
                pSettings->vstrCentralServerAddress[iLEIdx],
                iLEIdx );
        }
    }
}
