...
#define PROTMESSID_REQ_NETW_TRANSPORT_PROPS                                    \
    21                              // request properties for network transport
...
#define PROTMESSID_MUTE_STATE_CHANGED                                          \
    31 // mute state of your signal at another client has changed
#define PROTMESSID_CLIENT_ID 32 // current user ID and server status
#define PROTMESSID_RECORDER_STATE                                              \
    33 // contains the state of the jam recorder (ERecorderState)
#define PROTMESSID_REQ_SPLIT_MESS_SUPPORT                                      \
    34                                     // request support for split messages
...
#define PROTMESSID_CLM_PING_MS_WITHNUMCLIENTS                                  \
    1002 // for ping time and num. of clients info
...
#define PROTMESSID_CLM_VERSION_AND_OS                                          \
    1011 // version number and operating system
#define PROTMESSID_CLM_REQ_VERSION_AND_OS                                      \
    1012 // request version number and operating system
#define PROTMESSID_CLM_CONN_CLIENTS_LIST                                       \
    1013 // channel infos for connected clients
#define PROTMESSID_CLM_REQ_CONN_CLIENTS_LIST                                   \
    1014 // request the connected clients list
#define PROTMESSID_CLM_CHANNEL_LEVEL_LIST 1015 // channel level list
#define PROTMESSID_CLM_REGISTER_SERVER_RESP                                    \
    1016 // status of server registration request
#define PROTMESSID_CLM_REGISTER_SERVER_EX                                      \
    1017 // register server with extended information
...
#define MAX_NUM_MESS_SPLIT_PARTS                                               \
    ( MAX_SIZE_BYTES_NETW_BUF / MESS_SPLIT_PART_SIZE_BYTES )
...
class CProtocol : public QObject
{
  ...
  public:
    ...
    void Reset();
    void SetSplitMessageSupported ( const bool bIn )
    {
        bSplitMessageSupported = bIn;
    }
    ...
    void CreateMuteStateHasChangedMes ( const int  iChanID,
                                        const bool bIsMuted );
    ...
    void
         CreateCLConnClientsListMes ( const CHostAddress&          InetAddr,
    ...
    void
    ParseConnectionLessMessageBody ( const CVector<uint8_t>& vecbyMesBodyData,
                                     const int               iRecID,
  ...
  protected:
    class CSendMessage
    {
      public:
        CSendMessage() :
            vecMessage ( 0 ),
            iID ( PROTMESSID_ILLEGAL ),
            iCnt ( 0 )
        {
        }
        CSendMessage ( const CVector<uint8_t>& nMess,
                       const int               iNCnt,
                       const int               iNID ) :
            vecMessage ( nMess ),
            iID ( iNID ),
    ...
    void PutStringUTF8OnStream (
        CVector<uint8_t>& vecIn,
        int&              iPos,
    ...
    bool GetStringFromStream (
        const CVector<uint8_t>& vecIn,
        int&                    iPos,
  ...
  signals:
    ...
    void VersionAndOSReceived ( COSUtil::EOpSystemType eOSType,
                                QString                strVersion );
