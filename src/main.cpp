    ...
    for ( int i = 1; i < argc; i++ )
    {
        ...
        if ( GetFlagArgument ( argv, i, "-F", "--fastupdate" ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- using %1 samples frame size mode" )
                                  ...
                                  rDbleArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- maximum number of channels: %1" )
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- MIDI controller settings: %1" )
                    .arg ( strMIDISetup ) );
                                  ...
                                  rDbleArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- selected port number: %1" ).arg ( iPortNumber ) );
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- HTML status file name: %1" )
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- client name: %1" ).arg ( strClientName ) );
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- recording directory name: %1" )
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- central server: %1" ).arg ( strCentralServer ) );
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- welcome message: %1" ).arg ( strWelcomeMessage ) );
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- initialization file name: %1" )
                    .arg ( strIniFileName ) );
                                 ...
                                 strArgument ) )
        {
            ...
            qInfo() << qUtf8Printable (
                QString ( "- connect on startup to address: %1" )
        ...
        qCritical() << qUtf8Printable (
            QString ( "%1: Unknown option '%2' -- use '--help' for help" )
                .arg ( argv[0] )
                .arg ( argv[i] ) );
...
// is called with weird command line args -> do not exit on these
    ...
    if ( bIsClient && bUseGUI && bMuteMeInPersonalMix )
    {
        ...
        qWarning() << "Mute my own signal in my personal mix is only supported "
                      "in headless mode.";
    }
...
#else
    QCoreApplication* pApp = bUseGUI ? new QApplication ( argc, argv )
    ...
    auto result = QtAndroid::checkPermission (
        QString ( "android.permission.RECORD_AUDIO" ) );

    if ( result == QtAndroid::PermissionResult::Denied )
    {
        QtAndroid::PermissionResultMap resultHash =
            QtAndroid::requestPermissionsSync (
                QStringList ( { "android.permission.RECORD_AUDIO" } ) );

        if ( resultHash["android.permission.RECORD_AUDIO"] ==
...
QString UsageArguments ( char** argv )
{
    return "Usage: " + QString ( argv[0] ) +
           ...
           "  -t, --notranslation   disable translation (use English "
           "language)\n"
           "  -v, --version         output version information and exit\n"
           "\nServer only:\n"
           "  -d, --discononquit    disconnect all clients on quit\n"
           "  -e, --centralserver   address of the server list on which to "
           "register\n"
           "                        (or 'localhost' to be a server list)\n"
           "  -f, --listfilter      server list whitelist filter in the "
           "format:\n"
           "                        [IP address 1];[IP address 2];[IP address "
           "3]; ...\n"
           "  -F, --fastupdate      use 64 samples frame size mode\n"
           "  -l, --log             enable logging, set file name\n"
           "  -L, --licence         show an agreement window before users can "
           "connect\n"
           "  -m, --htmlstatus      enable HTML status file, set file name\n"
           "  -o, --serverinfo      infos of this server in the format:\n"
           "                        [name];[city];[country as QLocale ID]\n"
           "  -R, --recording       sets directory to contain recorded jams\n"
           "      --norecord        disables recording (when enabled by "
           "default by -R)\n"
           ...
           "      --mutemyown       mute me in my personal mix (headless "
           "only)\n"
           "  -c, --connect         connect to given server address on "
           "startup\n"
           "  -j, --nojackconnect   disable auto Jack connections\n"
           "      --ctrlmidich      MIDI controller channel to listen\n"
           "      --clientname      client name (window title and jack client "
           "name)\n"
           "\nExample: " +
                         ...
                         QString& strArg )
{
         ...
         ( !strLongOpt.compare ( argv[i] ) ) )
    {
        ...
        {
            qCritical() << qUtf8Printable (
                QString ( "%1: '%2' needs a string argument." )
                    .arg ( argv[0] )
                    .arg ( strLongOpt ) );
            exit ( 1 );
        }
                          ...
                          double& rValue )
{
    ...
    {
        QString errmsg =
            "%1: '%2' needs a numeric argument between '%3' and '%4'.";
        if ( ++i >= argc )
        {
            qCritical() << qUtf8Printable ( errmsg.arg ( argv[0] )
                                                .arg ( strLongOpt )
                                                .arg ( rRangeStart )
                                                .arg ( rRangeStop ) );
            exit ( 1 );
