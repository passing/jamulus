...
#define AUTO_NET_BUF_SIZE_FOR_PROTOCOL                                         \
    ( MAX_NET_BUF_SIZE_NUM_BL +                                                \
      1 ) // auto set parameter (only used for protocol)
...
#define MAX_NUMBER_SOUND_CARDS                                                 \
    129 // e.g. 16 inputs, 8 outputs + default entry (MacOS)
...
#define CHANNEL_LEVEL_UPDATE_INTERVAL                                          \
    200 // number of frames at 64 samples frame size

// time-out until a registered server is deleted from the server list if no
// new registering was made in minutes
#define SERVLIST_TIME_OUT_MINUTES                                              \
    33 // minutes (should include 3 UDP registration messages)
...
#define SERVLIST_TIME_PERMSERV_MINUTES                                         \
    2880 // minutes, 2880 = 60 min * 24 h * 2 d
